...
namespace Common.Authenticators.Test
{
    public class ServicePrincipalAuthenticatorTests
    {
        ...
        [Fact]
        [Trait(Category.AcceptanceType, Category.CheckIn)]
        public async Task ServicePrincipalCertificateFileNotExistuthenticationTest()
        {
            var accountId = "testuser";
            var certificateFile = "d:/certficatefortest.pem";
            var thumbprint = Guid.NewGuid().ToString();
            var securePassword = new SecureString();
            "pa88w0rd!".ToCharArray().ForEach(c => securePassword.AppendChar(c));
            ...
            AzureSession.Instance.DataStore = new MockDataStore();
            //var certificate = AzureSession.Instance.DataStore.GetCertificate(thumbprint);
            //AzureSession.Instance.DataStore.GetFiles(certificateFile);
            ...
            //Run
            var authenticator = new ServicePrincipalAuthenticator();
            await Assert.ThrowsAsync<InvalidOperationException>(() => authenticator.Authenticate(parameter));

            //Verify
            mockAzureCredentialFactory.Verify(f => f.CreateClientCertificateCredential(TestTenantId, accountId, It.IsAny<X509Certificate2>(), It.IsAny<ClientCertificateCredentialOptions>()), Times.Never);
            AzureSession.Instance.DataStore = prevDataStore;
        }
    }
}
